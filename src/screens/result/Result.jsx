...
function Result() {
  ...
  const [downFailed, setDownFailed] = useState(false);
  ...
  const downloadImage = async (url, fileName) => {
    try {
      ...
      const response = await axios({
        url,
        method: "GET",
        responseType: "blob", // Use blob for binary data in the browser
      });
  ...
  const fetchImages = async (phnNo) => {
    try {
      ...
      setDetectedImages((prevDetectedImages) => {
        const newImages = res.filter(
          (newImage) =>
            !prevDetectedImages.some(
              (existingImage) => existingImage.image_url === newImage.image_url
            )
        );

        if (newImages.length > 0) {
          return [...prevDetectedImages, ...newImages];
        }
        return prevDetectedImages;
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeFetchImages = async () => {
      if (eventID != null) {
        if (isRegistered === false || isRegistered === true) {
          if (!isRegistered) {
            if (fname && lname && email && img && phnNo) {
              ...
              await handleUploadApi(formData, phnNo);
            }
          } else {
            if (phnNo) {
              setWithExpiry("phnNo", phnNo, 86400000);
              await fetchImages(phnNo);
            }
          }
        } else {
          const phnNoLS = getWithExpiry("phnNo");
          if (phnNoLS != null) {
            await fetchImages(phnNoLS);
          } else {
            navigate("/");
            return;
          }
      ...
      intervalId = setInterval(() => {
        const currentPhnNo = getWithExpiry("phnNo");
        if (currentPhnNo) {
          fetchImages(currentPhnNo);
        }
      }, 10 * 1000);
    };

    initializeFetchImages();
          ...
          <>
            <div className="pt-4 d-flex justify-content-between ">
              <div className="d-flex gap-4">
                <h3 className="text-white miama-font fs-1">
                  Your result ({detectedImages.length}){" "}
                </h3>
                {spinner ? (
